...
while getopts "e:sd" opt; do
  case $opt in
    e)
      #echo "Setting admin contact email to: $OPTARG" >&2
      BES_HELP_EMAIL=$OPTARG
      ;;
    s)
...
#
if [ $BES_HELP_EMAIL != "not_set" ]; then
    echo "Setting Admin Contact To: $BES_HELP_EMAIL"
    sed -i "s/admin.email.address@your.domain.name/$BES_HELP_EMAIL/" /etc/bes/bes.conf
fi
if [ $BES_SYMLINKS != "not_set" ]; then
    echo "Setting FollowSymLinks to YES."
