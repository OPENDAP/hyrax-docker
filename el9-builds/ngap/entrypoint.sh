...
if test -n "$HOST" && test -n "$USERNAME" && test -n "$PASSWORD"; then
  ...
  # machine is a domain name or an ip address, not a URL.
...
# not empty and use its value if present and non-empty.olfs
#
...
if test -n "$USER_ACCESS_XML"; then
  ...
  # startup_log ( cat "$USER_ACCESS_XML_FILE" )
fi
################################################################################
...
if test -n "$BES_SITE_CONF"; then
  ...
  # startup_log $( cat "$BES_SITE_CONF_FILE" )
fi
#
...
if test -n "$TOMCAT_CONTEXT_XML"; then
  ...
  # startup_log "$(cat "$TOMCAT_CONTEXT_FILE" )"
fi
################################################################################
...
if test -n "$NGAP_CERTIFICATE"; then
  ...
  # startup_log "$(cat "$NGAP_CERTIFICATE_FILE" )"
fi
################################################################################
...
if test -n "$NGAP_CERTIFICATE_KEY"; then
  ...
  # startup_log "$(cat "$NGAP_CERTIFICATE_KEY_FILE" )"
fi
################################################################################
...
if test -n "$TOMCAT_REDISSON_XML"; then
  ...
  startup_log "$(cat "$TOMCAT_REDISSON_FILE" )"
fi
################################################################################
...
while getopts "e:sn:dk:i:r:" opt; do
  startup_log "Processing command line opt: $opt"
  case "$opt" in
  d)
    export debug="true"
    startup_log "Set debug: $debug"
    ;;
  e)
  ...
  \?)
    ...
    echo "    of the ncWMS service (for example https://foo.com:8090)." >&2
...
while /bin/true; do
  ...
  if test $BESD_STATUS -ne 0; then
    error_log "BESD_STATUS: $BESD_STATUS besd_pid:$besd_pid"
