...
if [ $debug = true ];then
    ...
    jar -tf "$CATALINA_HOME/webapps/opendap.war"  >&2
    ls -l "$CATALINA_HOME/webapps/opendap/"  >&2
    ls -l "$CATALINA_HOME/webapps/opendap/WEB-INF/"  >&2
    ls -l "$CATALINA_HOME/webapps/opendap/WEB-INF/conf/"  >&2
fi
...
if [ $SERVER_HELP_EMAIL != "not_set" ]; then
    echo "BES: Setting Admin Contact To: $SERVER_HELP_EMAIL"
